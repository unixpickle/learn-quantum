...
func H(c Computer, bitIdx int) {
	...
	c.Unitary(bitIdx, &Matrix2{s, s, s, -s})
}
...
func SqrtT(c Computer, bitIdx int) {
	c.Unitary(bitIdx, &Matrix2{1, 0, 0, cmplx.Exp(complex(0, math.Pi/8))})
}
...
func X(c Computer, bitIdx int) {
	c.Unitary(bitIdx, &Matrix2{0, 1, 1, 0})
}

// Y performs a Pauli Y-gate.
func Y(c Computer, bitIdx int) {
	c.Unitary(bitIdx, &Matrix2{0, complex(0, -1), complex(0, 1), 0})
}

// Z performs a Pauli Z-gate.
func Z(c Computer, bitIdx int) {
	c.Unitary(bitIdx, &Matrix2{1, 0, 0, -1})
}
